...
from utils.math import get_angle_between_dots
...
class Line(CollideInterface):
    ...
    def move_to(self, xy: Vector2DType, *args, **kwargs) -> None:
        self._change_position(xy, *args, **kwargs)
    ...
    def collide_dots(self, dots) -> bool:
        for dot in dots:
            if self.collide_point(dot):
                return 1
        return 0
    ...
    def collide(self, other) -> bool:
        if self._collide_able:
            if other.FORM_TYPE == self.SHAPE_TYPE:
                return self.collide_line(other)

            elif other.FORM_TYPE == ShapeType.Rect:
                return self.collide_rect(rect_obj=other)

            elif other.FORM_TYPE == ShapeType.Circle:
