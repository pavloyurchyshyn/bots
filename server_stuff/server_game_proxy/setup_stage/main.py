...
class GameSetup:
    ...
    def choose_map(self, request: dict, client: Client, **kwargs):
        Global.logger.info(f'{client.token} changing map.')
        if client.is_admin:
            ...
            if map_ + 1 > len(self.game_logic.maps_mngr.maps):
                Global.logger.warning(f'Player {client.token} asking for bad map id {map_}')
                return

            # TODO check for maps count
            Global.logger.info(f'{client.token} changing map to {map_}')
            ...
            Global.logger.debug(f'Player {client.token} is not admin!')

    def start_match(self, request: dict, client: Client, **kwargs):
        if client.is_admin:
            if request[SSR.Player.StartMatch] and not self.game_server.started_match:
                Global.logger.info(f'Game data: {request}')
                self.game_server.start_game_match()

        else:
            Global.logger.warning(f'Non admin {client.token} asking for start')
